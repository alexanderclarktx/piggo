...
`range = require 'lib.lua-websocket.ext.range'`
`range(a[, b[, c]])` = A very simple function for creating tables of numeric for-loops.

### reload.lua: 

`reload = require 'lib.lua-websocket.ext.reload'`
...
`tolua = require 'lib.lua-websocket.ext.tolua'`
...
Notice that, calling `require 'lib.lua-websocket.ext'` will also call `getCmdline` on `arg`, producing the global `cmdline`.
