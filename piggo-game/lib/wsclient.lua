...
function _M:read()
    ...
    repeat
        ...
        if err=="closed" then
            log:info("ERR CLOSED ON WSCLIENT READ")
            return nil, nil, err
        end
        if part or res then
...
function _M:update()
    -- if self.status ~= 0 then log:info(STATUS[tostring(self.status)]) end
    local sock = self.socket
    -- log:info(self.status)
    if self.status==STATUS.TCPOPENING then
        local url = self.url
        local z, err = sock:connect(url.host, url.port)
        log:info(z)
        log:info(err)
        ...
        elseif self._length>600 then
            self:onerror("STATUS CLOSED")
            self.status = STATUS.CLOSED
        end
    elseif self.status==STATUS.CONNECTING then
        local res, e, partial = sock:receive("*l")
        if res then
            repeat
                log:info("CONNECTING res" .. res)
                res = sock:receive("*l")
            until res == ""
            self:onopen()
            self.status = STATUS.OPEN
        elseif e then
            -- log:info("CONNECTING e" .. e)
            if e == "closed" then
                self.status = STATUS.CLOSED
            end
        end
